...
temperature by fitting the two-component model to \PLANCK~217, 353, 545, and 
857 GHz along with  DIRBE/\IRAS~100$\mu$m data. Because our two-component model
...
foreground predictions from 100 GHz to 3000 GHz. We find that, in diffuse sky 
regions, our two-component 100-217 GHz thermal dust emission predictions are on
...
at visible wavelengths, and compare the resulting reddening estimates with 
...
\PLANCK~353 GHz, 545 GHz, 857 GHz and DIRBE/\IRAS~100$\mu$m emission with a 
single-MBB spectrum. Here we investigate the FDS99 two-component dust emission 
model as an alernative parametrization for the dust spectral energy 
distribution (SED) composed  of \PLANCK~High Frequency Instrument (HFI), DIRBE 
and \IRAS~data. In doing so, we obtain maps of dust temperature and optical 
depth, both at $6.1'$ resolution. Because we employ a model that has been 
validated with FIRAS data down to submillimeter (submm) wavelengths, we expect 
our best-fit parameters to be useful in constructing high-resolution 
...
derived optical depth map will serve as a valuable cross-check for extinction 
...
fitting. % reference fitting section at end of this sentence ?
...
by comparison to the FDS99 857 GHz prediction. This renders our \PLANCK~857 GHz
absolute zero level tied indirectly to H\,\textsc{i} through the FDS99 
100$\mu$m and 240$\mu$m zero levels.
...
correlation before and after correcting for the best-fit offset and residual 
dipole, for each $\nu_i \in$ \{143, 217, 353, 545\} GHz. Not only are the 
tightened correlations striking in these scatter plots, but the residual dipole
...
%Again, since DIRBE-quoted fluxes also follow the 
...
$\beta$=1.67, 2.7, 1.63, 2.82 ..., see Table \ref{tab:global}). This color 
correction approach makes the MCMC sampling described in $\S$\ref{sec:mcmc} 
much more computationally efficient by circumventing the need to perform the 
integral in the numerator of Equation \ref{equ:bpcorr} on-the-fly for each 
proposed dust temperature. We have chosen to compute the color corrections on a
per-MBB basis because this approach is very versatile; all possible 
two-component (and single-MBB) models are linear combinations of MBBs, so we 
can apply all of our color correction machinery even when we allow parameters 
other than temperature (e.g. $f_1$) to vary and thereby modify the dust 
spectrum shape.
...
values are order unity shows that the assumed uncertainties are not grossly 
...
based on \PLANCK~217-857 GHz and SFD 100$\mu$m at full $6.1'$ resolution. Note 
that the two lowest-frequency data points were not used in the two-component 
fit shown, while the three lowest-frequency data points were not used in the 
\cite{planckdust} single-MBB fit shown (cyan).}
\end{center}
\end{figure}
...
pixels, based on \PLANCK~217-857 GHz and SFD 100$\mu$m at full $6.1'$ 
...
$T_2$, we compute the corresponding value of $T_1$, thereby determining the 
temperature and emissivity power law index for both MBB components and thus 
fully specifying the SED shape. In principle, we could sample in either $T_1$ 
or $T_2$, but have chosen to sample in $T_2$ because emission from this 
component dominates in the relatively high frequency bands which most strongly 
constrain the dust temperatures.
...
\begin{equation} \label{equ:like}
\mathcal{L}(\mathbf{I}|\tilde{I}_{545}, T_2) = \textrm{exp}\Big[-\frac{1}{2}(\mathbf{I}-\mathbf{\tilde{I}})\Sigma^{-1}(\mathbf{I}-\mathbf{\tilde{I}})^T\Big]
\end{equation}
...
% need to make sure to cite planckcalib paper
...
% mention that each of the error terms is always rescaled properly according
% to smoothing/rebinning and relevant power spectrum
...
$\langle \tilde{I}_{545, j}/b_{545}(T_{2,j}) \rangle$, its variance, and the 
545 GHz optical depth $\tau_{545}$=$\langle \tau_{545, j} \rangle$ = 
...
Galactic latitude, we find a typical $T_2$ uncertainty of 0.45 K, and
typical $\tilde{I}_{545}$ fractional uncertainty of 13\%. Figure \ref{fig:sed}
shows a summary of the observed SED and resulting best-fit two-component
model for a single $N_{side}$=2048 pixel, and Figure \ref{fig:post} 
shows posterior PDFs for three pixels. Figure \ref{fig:comparison} shows
a map of our derived hot dust temperature at full-resolution, for a patch
of sky in the Polaris flare region.
...
Galactic latitude, the relatively low $f_1$ values in the Polaris flare region,
and the clouds with relatively high $f_1$ near the north Galactic pole.}
\end{center}
\end{figure}
...
$\beta$=1.57 providing the best fit. Horizontal lines indicate $\chi^2_{\nu}$ 
for other dust emission models considered, including the FDS99 best-fit 
two-component model (Table \ref{tab:global}, model 1, red) and the 
...
and the present goodness-of-fit analysis is especially encouraging for two main
reasons: (1) in the present analysis, dust temperature has been allowed to vary
on degree scales, whereas in $\S$\ref{sec:global} we assumed a single global 
dust temperature and (2) in the present analysis, our zero-level offsets factor
into the dust temperature, whereas in $\S$6 this was not the case, meaning the 
former and latter analyses agree in spite of their potential to be affected by 
rather different systematics.
...
two-component $\tau_{545}$, illustrating our procedure for calibrating optical 
depth to reddening.}
\end{center}
\end{figure}
...
but cannot completely resolve the systematic trends in reddening residuals.
...
% mention that DR includes software to obtain emission and extinction 
% predictions
...
map of best-fit 545 GHz optical depth.}
\end{center}
\end{figure*}
...
two-component dust emission model remains favored over single-MBB models when 
swapping the \PLANCK~HFI maps for FIRAS at frequencies below 1250 GHz. We 
compared dust SED models in two ways (1) by fitting a 100-3000 GHz spectrum 
composed of per-band correlation slopes versus \PLANCK~857 GHz (2) by finding 
the best-fit dust temperature and optical depth per line-of-sight, with each 
pixel's SED comprised of 100-3000 GHz \PLANCK+DIRBE data, and comparing the 
average goodness-of-fit under various emission models.
...
$\S$\ref{sec:hier} is formally of enormous significance, given the 
$\sim$75,000 degrees of freedom in that analysis.
...
single-MBB model systematically understimates low-frequency dust emission by 
...
% could add a sentence noting that similar zodi corrections are evident
% in all existing, comparable temperature map data products : Planck 2013a,
% SFD98 and FDS99
...
% perhaps zodi will just magically be fixed in the forthcoming Planck
