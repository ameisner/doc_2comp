...
constant, RMS value for the CMB amplitude, $\sigma_{CMB}$. Table 
\ref{table:offs} lists the adopted values of $\sigma_{CMB}$. $\delta O_{\nu_i}$
represents the uncertainty in the band $\nu_i$ zero level offset, and the 
values of $\delta O_{\nu_i}$ can be read off from the second column of Table 
\ref{table:offs}. $n_{\nu_i}$ represents the instrumental noise in band 
$\nu_i$. Because using per-pixel noise estimates based on the \verb|ii_cov| 
parameter can imprint sharp-edged features of the survey pattern onto the 
derived parameters, we have adopted a conservative, spatially constant value of
$n_{\nu_i}$ for each band. Table \ref{table:offs} lists the values of 
$n_{\nu_i}$ adopted. The next term accounts for the uncertainty on the CO 
emission correction, taking $\sigma_{CO, \nu_i}$=0.15$\times$$I_{CO, \nu_i}$ 
