...
% should really change labels to have **script** I to be consistent with the
...
overconstrained linear model with three parameters: a, b, and c. $a$ represents
the best-fit slope of \PLANCK~band $\nu_i$ versus \PLANCK~857 GHz assuming 
they are linearly related. $b$ represents the scaling of the Solar dipole that 
makes the \PLANCK~band $\nu_i$ versus 857 GHz correlation most tightly linear. 
$c$ represents the constant offset necessary to make the zero level of the 
band $\nu_i$ map consistent with that of the 857 GHz map. For each band
$\nu_i$, we obtain estimates of $a$, $b$, and $c$ by performing a linear least 
squares fit with uniform weights and iterative outlier rejection. Figure 
...
best-fit $c$ and $b\mathcal{D}$ from each band $\nu_i$ map. The best-fit
offsets and residual dipole amplitudes are listed in Table \ref{table:offs}.

% mention the special case of 100 GHz w/ quadrupole term !!!!!!
% do i need another equation ?
