...
$\sigma_{T_2}$=0.25 K and 
$\sigma_{\tilde{I}_{545}}$=max(0.01$\times$$I_{545}$, 0.05 MJy/sr). We run 5 
iterations of burn-in, each consisting of 500 MH steps. After each burn-in 
iteration, we rescale the proposal distribution so as to ultimately attain an 
acceptance fraction $f_{acc}$ as close as possible to the optimal value 
$f_{opt}=0.234$. This is accomplished by multiplying the proposal distribution 
standard deviations by $f_{acc}$/$f_{opt}$.

We then estimate the parameters and their covariances by performing 10000 steps
during which we keep track of the proposed $T_{2, j}$ and $\tilde{I}_{545, j}$ 
values at the $j^{th}$ step since the end of burn-in. From these 2000 samples, 
we compute estimates of each parameter, $T_2$ = $\langle T_{2, j} \rangle$, 
$\tilde{I}_{545}$ = $\langle \tilde{I}_{545, j} \rangle$, and of each 
...
corresponding estimate of the monochromatic two-component intensity at 545 GHz,
$M_{545}$ = $\langle M_{545, j} \rangle$ = 
