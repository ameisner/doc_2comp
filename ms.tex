...
($c_{\nu}$$\times$\verb|SMICA|($p$))$^2$, i.e. a term proportional to CMB model
amplitude. Because this error term's spatial dependence has the potential to 
imprint the CMB anisotropies on the derived parameters, we have chosen to 
replace \verb|SMICA|($p$) with a spatially constant, RMS value for the CMB 
amplitude, $\sigma_{CMB}$. Table \ref{table:offs} lists the adopted values of 
$\sigma_{CMB}$. $\delta O_{\nu_i}$ represents the uncertainty in the band 
$\nu_i$ zero level offset, and the values of $\delta O_{\nu_i}$ can be read 
off from the second column of Table \ref{table:offs}. $n_{\nu_i}$ represents 
