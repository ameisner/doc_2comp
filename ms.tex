...
contamination from each band in order to best isolate the thermal continuum we 
wish to characterize. The most prominent molecular line emission in the
\PLANCK~bands of interest arises from the three lowest CO rotational
...
353 GHz bands. J=1$\rightarrow$0 does also impart some signal upon \PLANCK~143 
GHz, but at a negligible level, $\sim$1000$\times$ fainter relative to the
dust continuum signal than at 100 GHz
...
We must apply the appropriate unit conversions to the PSF-matched Type 3 CO
...
is natively provided in units of K$_{RJ}$ km/s of J=1$\rightarrow$0 emission. 
To convert this quantity to $K_{CMB}$, we assume that all of the CO emission 
arises from the $^{12}$CO isotope, and use the $^{12}$CO conversion factors 
from \cite{planckco} as follows:
...
% state what I_3 is 
Where $I_{CO, \nu_i, N, N-1}$ is the intensity in $K_{CMB}$ due to 
CO emission in \PLANCK~band $\nu_i$ due to the CO transition from J=$N$ to
J=$(N$$-$1). The $F_{12CO, \nu_i, N, N-1}$ are taken to be PP, QQ, RR. 
...
In fact, for each of the \PLANCK~bands in which molecular emission is 
...
and the vast majority of the sky has completely neglibible CO emission. Thus,
...
map. We identify  low-signal regions as those with $\mathcal{I}_3$$<$1 K km/s, 
where $\mathcal{I}_3$ is the Type 3 CO map smoothed to 0.25$^{\circ}$ 
resolution. This cut results in 90\% of the sky being unaffected by the CO 
correction, particularly the vast majority of high Galactic latitude regions.
