...
opportunity to cross-check the conclusions of the correlation slope analysis in
...
$\chi^2_p$, is simply $-2 \times log(P_{max})$. 
% maybe invent a term chi^2_p or something to 
% reduce potential confusion caused by notation

\begin{figure}
\begin{center}
\epsfig{file=chi2_dirbe.eps, width=3.3in}
\caption{\label{fig:chi2_dirbe} Comparison of goodness-of-fit, 
$\chi^2_{\nu}$=$\langle \chi^2_p \rangle$ for various dust SED models.}
\end{center}
\end{figure}
...
a particular dust SED model is $\langle \chi^2_p \rangle$, where the average 
is taken over the aforementioned set of $\sim$10,800 pixels. 
$\langle \chi_{p}^2 \rangle$ is also equivalent to the reduced chi-squared, 
$\chi^2_{\nu}$, when considering the total number of free parameters to be the 
number of pixels multiplied by the number of free parameters per pixel, and 
similarly for the total number of constraints.
...
$\beta$=1.59. We also evaluated $chi^2_{\nu}$ for single-MBB models in which
$\beta$ varies spatially. In these cases, we used the $\beta$ map from 
\cite{planckdust}. We started by calculating $\chi^2_{\nu}$ using the 
\cite{planckdust} temperature map, finding $chi^2_{\nu}$=4.68. Note that
in this case no per-pixel chi-squared minimization was involved, as we simply 
evaluated $\chi^2_{p}$ for each pixel based on the fully-specified 
\cite{planckdust} model. % mention beta template but floating t, normalization

% mention other single MBB variants

Next, we calculate $\chi^2_{\nu}$ for the two-component model with global 
parameters from FDS99 (Table \ref{tab:global}, model 2). We find 
$\chi^2_{\nu}$=2.33, a slight improvement relative to the $\beta$=1.59 
single-MBB model. Finally, calculate $\chi^2_{\nu}$ for Table \ref{tab:global} 
model 3, the two-component model favored by our \PLANCK+DIRBE correlation 
slopes. In this case, we achieve the best goodness-of-fit out of the all the 
models tested, with $\chi^2_{\nu}$=2.11.

% end with a few sentences summarizing overall takeaway(s) from this 
% goodness-of-fit analysis

% could compare temperatures, but masks are different

% also should emphasize that an important difference here relative to the
% correlation slope analysis is that here, the zero levels actually do matter
