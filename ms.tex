...
%independent of the \PLANCK~545 GHz bandpass. Finally, we acompute the 545 GHz
%optical depth as $\tau_{545}$ = and its variance, with the source 
...
spatially constant. In principle, we could alternatively introduce a third free
 parameter by allowing $q_1/q_2$, $\beta_1$ or $\beta_2$ to vary while holding 
...
variation can be attributed to continuous variation of the dust species' mass
...
% sentence or two about why f_1 is natural parameter to try varying 
% rather than q1/q2 or beta1 or beta2 !!!!!~~!!!

In order for our variable $f_1$ fits to remain sufficiently constrained with 
the addition of another free parameter, we consider \PLANCK~maps smoothed to 
$1^{\circ}$ FWHM for increased S/N, and pixelize at $N_{side}=64$. To best
constrain the model parameters in each pixel, we also include \PLANCK~100 GHz 
and 143 GHz, and DIRBE 140$\mu$m and 240$\mu$m, all at $1^{\circ}$ resolution.
...
\ref{equ:t2prior} remains unchanged. We adopt the following prior on $f_1$:
...
%To boost S/N, we have performed
