...
to describe an observed SED: the hot dust temperature $T_2$ and the 
normalization of $M_{\nu}$. In principle, we could allow additional parameters,
 such as $f_1$ and the ratio $q_1/q_2$, to vary. However the number of free 
parameters in our baseline fit at the full $6.1'$ resolution is limited by the 
fact that, for each sky location, the observed SED contains only five 
independent intensity measurements (\verb|i100| along with the four 
highest-frequency \PLANCK~bands).
...
Use FIRAS data to justify $T_2$ prior.
