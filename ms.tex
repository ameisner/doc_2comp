...
separately solving two sub-problems: the relative offsets and low order
corrections derived by correlating the \PLANCK~bands against one another,
and second the absolute zero-level offset.
...
is set by requiring that \PLANCK~infrared emission tends to zero when 
...
unsatisfying in that there appear to be different slopes of 857 GHz versus
H\,\textsc{i} for different ranges of H\,\textsc{i}. In particular, 
\PLANCK~857 GHz appears to ``flatten out'' at very low H\,\textsc{i}. More 
quantitavely, we have found using the LAB H\,\textsc{i} data \citep{lab} for
$-72$$<$$v_{LSR}$$<$$+25$ that the best-fit slope for H\,\textsc{i}$<$70 K km/s
is a factor of $\sim$1.9 lower than the best fit slope for 
110 K km/s $<$H\,\textsc{i}$<$70 K km/s, and as a result the implied 
zero-point offsets for \PLANCK~857 GHz differ by $\sim$0.37 MJy/sr.

% CITE KALBERLA LAB PAPER !!!!!

Because of these ambiguities in the relationship between 857 GHz emission
and H\,\textsc{i}, we decided to instead constrain the 857 GHz zero level
by comparison to the FDS99 857 GHz prediction. This renders the 857 GHz 
...
Specifically, we fit a linear model to the \PLANCK~857 GHz values as a function
of FDS99 predicted 857 GHz. We used a version of the \PLANCK~857 GHz map with 
zodiacal light and point sources removed and smoothed to 1 degree resolution, 
which we will refer to as $\mathcal{I}_{857}$. For this analysis, we considered
$\mathcal{I}_{857}$ to be the independent variable, as it has much higher S/N 
than the FDS99 prediction, henceforward referred to as $\mathcal{F}_{857}$.
We then rebin to $N_{side}$=64 and restrict to pixels with 
$\mathcal{I}_{857}$$<$2.15 MJy/sr. Since \PLANCK~857 GHz smoothed to degree 
resolution has such high S/N, we can safely perform such a cut on 
$\mathcal{I}_{857}$. Figure XX shows the a scatterplot of $\mathcal{I}_{857}$
versus $\mathcal{F}_{857}$, with a moving median and linear fit overplotted.

% mention uniform weights in linear fit, but don't think i want an equation
% for linear fit since it's so simple to begin with
...
% mention use of bandpass correction factors when doing Planck vs. prediction
% regressions
