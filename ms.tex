...
Table 11 of \cite{planckcalib}. The second term is intended to represent a 
typical uncertainty due to the subtraction of the \verb|SMICA| CMB model. The 
analogous term in \cite{planckdust} Equation 7 is 
($c_{\nu}$$\times$\verb|SMICA|($p$))$^2$, i.e. an uncertainty proportional to 
the CMB model amplitude in each pixel. Because this term's spatial dependence 
has the potential to imprint the CMB anisotropies on the derived parameters, we
have chosen to replace \verb|SMICA|($p$) with a spatially constant, RMS value 
for the CMB amplitude, $\sigma_{CMB}$. Table \ref{table:offs} lists the adopted
values of $\sigma_{CMB}$. $\delta O_{\nu_i}$ represents the uncertainty in the 
band $\nu_i$ zero level offset, and the values of $\delta O_{\nu_i}$ can be 
read off from the second column of Table \ref{table:offs}. $n_{\nu_i}$ 
...
\PLANCK~intensity map. The next term accounts for the uncertainty on the 
CO emission correction, with $\sigma_{CO, \nu_i}$=0.15$\times$$I_{CO, \nu_i}$
(see $\S$\ref{sec:mole}, specifically Equation \ref{equ:molcorr}).

% still need to explain off-diagonal terms in covariance ...
...
and thermal dust emission ($\S$\ref{sec:lofreq}), respectively.
