...
account for the MBB($\beta$, $T$) spectral shape and the transmission as a 
...
tables. Here the single dimension is $T_k$, as we avoid the necessity of a 
two-dimensional interpolation in both $\beta$ and $T$ by computing a small set 
of one dimensional correction factors for the particular set of $\beta$ values
of interest ($\beta$$\in$\{1.67, 2.7, 1.19, 2.61\}, see Table \ref{tab:global}.

% maybe a final sentence to emphasize explicitly that this approach
% is much more computationally feasible than computing integrals
% on-the-fly every time a putative model is proposed (during sampling)

% maybe mention the motivation for computing b_{\nu_i} on a **per MBB**
% basis, the fact that this is more versatile, especially when 
% the ``global parameters'' like f_1 or others are being changed
