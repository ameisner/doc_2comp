...
% re: following sentence --- in the code nu_0 is actually 2997.92458
...
Following the convention of FDS99, $\nu_0$ is chosen to be 3000 GHz. In 
principle, one could imagine this two MBB model to be specified by seven free 
parameters for every line of sight: $T_1$, $T_2$, $\beta_1$, $\beta_2$, $f_1$, 
and $q_1$/$q_2$ and the normalization of $M_{\nu}$. However, as discussed in 
FDS99, $T_1$=$T_1$($T_2$, $\beta_1$, $\beta_2$, $q_1/q_2$) is fully specified 
by these other parameters under the assumption that the temperature of each 
species is determined by maintaining thermal equilibrium with the same ISRF. 
$T_1$ is always related to $T_2$ via a simple power law, although the prefactor
and exponent depend on the parameters $q_1/q_2$, $\beta_1$ and $\beta_2$. 
