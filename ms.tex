...
reprocessing of IRAS 100$\mu$m, which we will refer to as \verb|i100| (Schegel et al. 1998). \verb|i100| has angular resolution of $6.1'$. %list other
...
angular diameter $\sim$, defined by the HEALPix pixelization with nside=2048 \citep{healpix}. 
...
the normalization is controlled via $I^{*}_{545}$, the amplitude of the SED
in MJy/sr at $\nu$ = 545 GHz. For each pixel, we run a Metropolis-Hastings 
Markov chain sampling the posterior probability as a function of the two parameters $T_2$ and $I^{*}_{545}$. To be clear, we are sampling the posterior
given by:

\begin{equation}
P(I^{*}_{545}, T_2) \propto L({I_i}|I^{*}_{545}, T_2)P(T_2)
\end{equation}
