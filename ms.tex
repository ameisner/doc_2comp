...
\startdata
 ...
 3 & single MBB      &  1.0   &  ...  & 1.59 &  ... &  ...  & 19.63 & 0.999 & 6 & 33.9 & 5.65 \\ [-2ex]
\enddata
\end{deluxetable*}
...
analysis incorporating DIRBE and FIRAS data. In this section, we seek to 
perform a correlation slope determination of these same global parameters, but
swapping the \PLANCK~HFI maps for FIRAS at low frequencies, while still 
relying on DIRBE at higher frequencies. We also seek to determine via this
correlation slope analysis whether or not the combination of \PLANCK+DIRBE data
favors two-component models over single-MBB models in the same way that the
FIRAS+DIRBE data did in the FDS99 analysis.

In the two-component model case, we wish to obtain estimates for six 
free parameters: $\beta_1$, $\beta_2$, $q_1/q_2$, $f_1$, $T_2$ and the overall 
spectrum normalization $n$. The constraints employ are the correlation
slopes of each of the \PLANCK~HFI bands, as well as DIRBE 100$\mu$m (3000 GHz),
140$\mu$m (2141 GHz) and 240$\mu$m (1250 GHz) against \PLANCK~857 GHz. We will 
refer to the slope for band $\nu_i$ as $s_{i,857}$. All of the correlations are
performed using $1^{\circ}$ smoothed maps. The slopes for \PLANCK~100-545 GHz 
are taken to be those from the relative zero level fits of 
\ref{sec:relzero}, and are illustrated by the dashed red lines in plots of 
Figure \ref{fig:dip}, right column. The 857 GHz slope is taken to be unity by 
definition. We use the SFD98 reprocessings of the DIRBE bands. Including 857 
GHz, we thus have 9 correlation slope constraints for six free parameters. 
Including DIRBE 140$\mu$m and 240$\mu$m is critical in making this problem 
sufficiently constrained, and also in providing information near the peak of 
the dust SED at $\sim$140$\mu$m, which is particularly sensitive to the 
presence of a single versus multiple MBB components. % last sentence mentioning
...
We assume an uncertainty of 2\% on each of the 9 correlation slopes and
minimize the $\chi^2$ given by:
...
\begin{equation}
\chi^2 = \sum_{i=0}^{8}\frac{(s_{i, 857}-n\tilde{I}_{\nu_i}/\tilde{I}_{857})^2}{\sigma_{s}^2}
\end{equation}
