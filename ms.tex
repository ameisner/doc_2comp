...
(3). Because $f_1$ is so strongly covariant with $\beta_1$, the difference of 
$f_1$ between

The MCMC fitting procedure described in the following sections is performed
separately for both model 1 (FIRAS+\IRAS) and model 2 (\PLANCK+\IRAS), as 
defined by the global parameters in Table \ref{tab:global}.

% multi-panel figure showing more scatterplots of Planck vs. FDS predictions

% at the end of this section state that we do all of our fits etc. with
% both models (call one of them model 1, and the other model 2, similar
% to what FDS99 did
