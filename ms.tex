...
\caption{\label{fig:dirbe_slopes} DIRBE linear fits for correlation slope 
analysis.}
\end{center}
\end{figure*}
...
%All of the correlations are performed using 
%$1^{\circ}$ smoothed maps

% need to mention that this is all for the low HI mask !!!!
% should have a three panel figure illustrating linear fits of 
...
correlation slope analysis has confirmed the basic conclusion of FDS99 and 
others e.g. \cite{reach95}, that the FIR/submm dust SED prefers two MBBs to 
just one, but, for the first time, independent of FIRAS. However, it is also
apparent that the improvement in $\chi^2_{\nu}$ for single-MBB versus double
MBB models found here is substantially less dramatic 
($\Delta\chi^2_{\nu}$=4.32) than that found in FDS99 
($\Delta\chi^2_{\nu}$=29.2). This is likely attributable to the exquisite 
narrow-band frequency coverage of FIRAS, especially near the dust SED peak, 
which makes FIRAS a better suited data set than \PLANCK~for a detailed 
analysis of the globally best-fit dust SED model.

% acknowledge that the preference for two MBBs is relatively weak when swapping
% in Planck for FIRAS, probably due to the 
