...
to the observed sky intensity. At optical and ultraviolet wavelengths, 
...
knowledge about infrared emission from the ISM. \IRAS, with its $\sim$4$'$ 
resolution, revolutionized the study of Galactic dust emission, first revealing
 the high-latitude ``infrared cirrus'' using 60$\mu$m and 100$\mu$m 
observations \citep{low84, wheelock94}. Then, beginning in 1989, DIRBE provided
complementary full-sky measurements at ten infrared wavelengths from 
...
provided full-sky infrared dust spectra at $7^{\circ}$ resolution in 213 
narrow frequency bins between 200 GHz and 2100 GHz.
...
$\beta$ 
could not accurately match the \IRAS/\FIRAS/DIRBE spectrum at both the Wien and 
Rayleigh-Jeans extremes. To fit both ends of the spectrum, FDS99 proposed
a dust emission model consisting of two MBBs, each with a different
...
relatively artifact-free broadband data covering the entire sky and a broad 
...
doing so, we obtain $6.1'$ resolution maps of dust temperature and optical 
depth. Because
we are using a model that has been validated using FIRAS data down to sub-mm
wavelengths, e expect our best-fit parameters to be useful in constructing high-resolution
...
model may not adequately fit, and also data near the peak of the dust SED, for
 example \AKARI~140-160$\mu$m. We also anticipate our derived optical depth map
 will serve as a useful comparison for extinction estimates based 
...
model $T_2$, and \cite{planckdust} temperature (labeled $T_{R1.20}$). Note the differing colorscales.
