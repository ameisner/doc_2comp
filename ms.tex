...
spectrum. In practice, we retrieve $u_{\nu_i}(T, \beta)$ in a fashion similar 
to that used for the $b_{\nu_i}(T, \beta)$ color corrections, using a set of 
precomputed one-dimensional look-up tables. To reiterate, for $\nu_i$$\le$353 
GHz $u_{\nu_i}(T_k, \beta_k)$ has units of (MJy/sr)/$K_{CMB}$, and therefore 
$\tilde{I}_{\nu_{i}}$ will have different units depending on the band $\nu_i$, 
though $\tilde{I}_{\nu_{i}}$ always has the same units as the native band 
$\nu_i$ map.
