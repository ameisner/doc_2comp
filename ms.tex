...
all $\nu_i$$\ge$545 GHz. For $\nu_i$$\le$353 GHz, $u_{\nu_i}(T_k, \beta_k)$ 
represents the appropriate conversion factor from $K_{CMB}$ to MJy/sr:
...
a spectral index $\alpha$=$-1$ spectrum. The values of $u_{\nu_i, IRAS}$
are listed in the second column of \cite{planckresponse} Table 6. 
$u_{\nu_i, IRAS \ to \ (T, \beta)}$ is a color correction factor relative to
the \IRAS~convention for band $\nu_i$ and a MBB($T$, $\beta$) spectrum.
To reiterate, for $\nu_i$$\le$353 GHz $u_{\nu_i}(T_k, \beta_k)$ has units 
of (MJy/sr)/$K_{CMB}$, and therefore $I_{\nu_{i}}$ will have different 
