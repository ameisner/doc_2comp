...
% maybe a final sentence or two explaining that, to whatever extent
...
The primary FDS99 analysis considered the grain properties, for example the
emissivity power law indices $\beta$, to be constant over the sky, and 
performed a correlation slope analysis to constrain these parameters with
FIRAS and DIRBE observations. A DIRBE 240$\mu$m/100$\mu$m ratio was used
to account for temperature variation at $\sim$1.3$^{\circ}$ resolution. In this
work we seek to apply FDS99-like emission models to the \PLANCK~data set, which
offers a dramatic enhancement in angular resolution relative to the FIRAS 
spectra. The \PLANCK~data allow us to derive a temperature correction at 
near-\IRAS~resolution, re-evaluate the best-fit global dust properties, and
fit addition parameters of the two-component model as a function of position
on the sky.
...
The shape of the two-component spectrum we will consider is given by:
...
differ.In accordance with the conventions of FDS99, $\nu_0$ is chosen to be 
3000 GHz. In principle, one could imagine this two MBB model to be specified by
seven free parameters for every line of sight: $T_1$, $T_2$, $\beta_1$, 
$\beta_2$, $f_1$, and $q_1$/$q_2$ and the normalization of $M_{\nu}$. However, 
as discussed in FDS99, $T_1$=$T_1$($T_2$, $\beta_1$, $\beta_2$, $q_1/q_2$) is 
...
with the same ISRF. $T_1$ is always related to $T_2$ via a simple power law, 
although the prefactor and exponent depend on the parameters $q_1/q_2$, 
$\beta_1$ and $\beta_2$. $q_1/q_2$ encodes the relative emission/absorption 
cross-sections of the two dust species, while $f_1$ dictates the relative 
...
\begin{figure}
\begin{center}
\epsfig{file=ebv_resid.eps, width=2.5in, angle=90}
