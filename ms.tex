...
dipole amplitude. For each 100-545 GHz HFI band, we restrict tor regions of low
...
% adopted the zero-point offsets of \cite{planckdust} Table 1, which were 
% derived by correlating \PLANCK~857 GHz versus H\,\textsc{i} at low column 
% density, and
...
\mathcal{I}_{\nu_i, p} = a\mathcal{I}_{857, p} + b\mathcal{D}_{p} + c
\end{equation}
...
\{100, 143, 217, 353, 545\} GHz. $\mathcal{D}$ is a scaling of the Solar dipole
pattern oriented toward $(l, b) = (263.99^{\circ}, 48.26^{\circ})$, with unit 
amplitude. Because $\sim$18,000 pixels satisfy the low H\,\textsc{i} cut, we 
have an overconstrained linear model with three parameters: a, b, and c. $a$ 
represents the slope of \PLANCK~band $\nu_i$ GHz versus \PLANCK~857 GHz 
assuming they are linearly correlated. $b$ represents the scaling of the Solar 
dipole that makes the 857 GHz vs. \PLANCK~band $\nu_i$ correlation most tightly
linear, and $c$ represents the constant offset necessary to make the zero level
 of the band $\nu_i$ map consistent with that of the 857 GHz map. Performing a 
linear least squares fit with uniform weights and iterative outlier rejection, 
we find $a=4.33\times10^{-5}\ K/(MJy/sr)$, $b=-1.40\times10^{-5}\ K$, and 
...
%in above paragraph, clarify what dipole means i.e. in what direction
% on sky it's oriented

%mention the special case of 100 GHz w/ quadrupole term ... do i need
% another equation ?
...
\tablecolumns{5} 
\tablewidth{0pc} 
\tablecaption{\label{obs} Global Model Parameters} 
\tablehead{
\colhead{Data} &
\colhead{$f_1$} &
\colhead{$q_1/q_2$} & 
...
\PLANCK+\IRAS & 0.0267 & 3.68 & 1.19 & 2.61
\enddata
\end{deluxetable}

%%%%

\subsection{Correlation Slopes}
